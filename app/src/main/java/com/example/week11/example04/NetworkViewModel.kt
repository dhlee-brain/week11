...
class NewsViewModel : ViewModel() {
    ...
    fun fetchNews(url: String) {
        ...
        viewModelScope.launch {
            try {
                val fetchedNews = getRSSNews(url)
    ...
    private suspend fun getRSSNews(url: String): List<NewsData> = withContext(Dispatchers.IO) {
        val doc = Jsoup.connect(url).parser(Parser.xmlParser()).get()
        val headlines = doc.select("item")
        headlines.map {news-> NewsData(news.select("title").text(),
